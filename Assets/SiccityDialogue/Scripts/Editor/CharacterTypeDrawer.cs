...
using CharacterInfo = Dialogue.CharacterType;
...
namespace Dialogue {
	...
	public class CharacterTypeDrawer : PropertyDrawer {
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			...
			CharacterType currentCharInfo = property.objectReferenceValue as CharacterType;
			...
			if (GUI.Button(buttonRect, buttonLabel)) {
				...
				string[] guids = AssetDatabase.FindAssets("t:CharacterType");
				for (int i = 0; i < guids.Length; i++) {
					string path = AssetDatabase.GUIDToAssetPath(guids[i]);
					CharacterType matInfo = AssetDatabase.LoadAssetAtPath(path, typeof(CharacterType)) as CharacterType;
					if (matInfo != null) {
			...
			EditorGUI.ObjectField(position, property, typeof(CharacterType), GUIContent.none);
		...
		private void SelectMatInfo(SerializedProperty property, CharacterType charInfo) {
